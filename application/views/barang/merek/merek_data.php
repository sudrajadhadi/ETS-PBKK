...
<div class="container">
	<div class="panel panel-default">
		<div class="panel-body">
			<h5><i class='fa fa-cube fa-fw'></i> Barang <i class='fa fa-angle-right fa-fw'></i> List Merek</h5>
			<hr />
...
{
	$tambahan .= nbs(2)."<a href='".site_url('barang/tambah-merek')."' class='btn btn-default' id='TambahMerek'><i class='fa fa-plus fa-fw'></i> Tambah Merek</a>";
...
<script type="text/javascript" language="javascript" >
	$(document).ready(function() {
		var dataTable = $('#my-grid').DataTable( {
			...
			"ajax":{
				url :"<?php echo site_url('barang/list-merek-json'); ?>",
				type: "post",
				error: function(){ 
					$(".my-grid-error").html("");
	...
	$(document).on('click', '#TambahMerek, #EditMerek', function(e){
		e.preventDefault();

		$('.modal-dialog').addClass('modal-sm');
		$('.modal-dialog').removeClass('modal-lg');
		if($(this).attr('id') == 'TambahMerek')
		{
			$('#ModalHeader').html('Tambah Merek');
		}
		if($(this).attr('id') == 'EditMerek')
		{
			$('#ModalHeader').html('Edit Merek');
		}
